...
class NeuSRenderer:
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s):
        ...
        inside_sphere = (radius[:, :-1] < 5.0) | (radius[:, 1:] < 5.0)
    ...
    def render_core(self,
        ...
        inside_sphere = (pts_norm < 5.0).float().detach()
        # inside_sphere = torch.ones_like(inside_sphere).float().detach()
        relax_inside_sphere = (pts_norm < 5.2).float().detach()
    ...
    def render(self, rays_o, rays_d, near, far, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0, ts=None, test_time=False):
        device = torch.device('cuda')
