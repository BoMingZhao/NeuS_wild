...
class BlenderDataset(Dataset):
    def __init__(self, root_dir, split='train', img_wh=(800, 800),
        ...
        self.object_bbox_min = np.array([-2.2, -2.2, -2.2])
        self.object_bbox_max = np.array([2.2, 2.2, 2.2])
        self.n_images = 4
    ...
    def read_meta(self):
        ...
        if self.split == 'train': # create buffer of all rays and rgb data
            ...
            for t, frame in enumerate(self.meta['frames']):
                ...
                rays_t = 1 * torch.ones(len(rays_o), 1)
    ...
    def __getitem__(self, idx):
        if self.split == 'train': # use data in the buffers
            return self.all_rays[idx, :9], self.all_rgbs[idx]
            ...
            img = Image.open(os.path.join(self.root_dir, f"{frame['file_path']}.png")).convert('RGB')
            img = img.resize(self.img_wh, Image.LANCZOS)
            ...
            ts = 1 * torch.ones(len(rays_o), 1)
            # near, far = near_far_from_sphere(rays_o, rays_d)
            rays = torch.cat([rays_o, rays_d, 
                              self.near*torch.ones_like(rays_o[:, :1]),
                              self.far*torch.ones_like(rays_o[:, :1]), ts],
                              1) # (H*W, 8)

            sample = {'rays': rays,
                      'ts': 1 * torch.ones(len(rays), dtype=torch.long),
                      'rgbs': img,
                      'c2w': c2w}
